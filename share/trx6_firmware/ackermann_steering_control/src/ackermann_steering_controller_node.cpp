...
struct TwistToAackermann : public rclcpp::Node
{
  ...
  void callback(const ackermann_msgs::msg::AckermannDrive::SharedPtr msg)
  {
    ...
    if (msg->steering_angle == 0 || msg->speed == 0)
    {
      lwh = 0;
      rwh = 0;
      rwr = lwr = rwf = lwf = rwm = lwm = linearToAngularVelocity(msg->speed, wheelRadius);
      slip = 0;
    }
    else
    {
      // The radius of the turn.
      tanz = tan(msg->steering_angle);
      r = actualBase / tanz;
      slip = atan(tanz / 2);
      // ROS_DEBUG("%f\n", r);
      ...
      lwf = lwm = lwr = linearToAngularVelocity((r - (track / 2)) * msg->steering_angle, wheelRadius);
      rwf = rwm = rwr = linearToAngularVelocity((r + (track / 2)) * msg->steering_angle, wheelRadius);
    }
