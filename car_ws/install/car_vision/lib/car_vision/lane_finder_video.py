...
def main(args=None):
    ...
    pub = node.create_publisher(CurveCoeffs,"/lane_detection/curve",1)
    msg = CurveCoeffs()
    msg.degree = 0 #Start at stopping condition
    cap = cv2.VideoCapture("/home/tan/Desktop/car_d/car_ws/src/car_vision/scripts/solidWhiteRight.mp4")
    while(cap.isOpened()):
        _, frame = cap.read()
